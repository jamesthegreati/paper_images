...
# First, commit the base files (README, .gitignore, scripts)
...
if ($LASTEXITCODE -eq 0) {
    Write-Host "SUCCESS: Base files committed" -ForegroundColor Green
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "SUCCESS: Base files pushed" -ForegroundColor Green
    } else {
        Write-Host "ERROR: Failed to push base files" -ForegroundColor Red
        exit 1
    }
} else {
    Write-Host "INFO: No changes to base files or already committed" -ForegroundColor Gray
}
...
foreach ($year in $years) {
    ...
    if ($status) {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            if ($LASTEXITCODE -eq 0) {
                Write-Host "  SUCCESS: $year pushed" -ForegroundColor Green
            } else {
        ...
        Write-Host "  INFO: No changes for $year (already committed)" -ForegroundColor Gray
    }
...
Write-Host ""
Write-Host "Verify upload at:" -ForegroundColor Yellow
